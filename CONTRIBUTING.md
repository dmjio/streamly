...
2. In general we must not share the SVar and everytime we pass on the state to
   run a stream we must use `rstState` to reset the SVar in the state.
...
to be careful about it when coding.
